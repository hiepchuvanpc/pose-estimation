...
import numpy as np
import os
from dtw_standalone.core.pose_tracker import PoseTracker
from dtw_standalone.core.normalizer import PoseNormalizer
from dtw_standalone.core.features import FeatureExtractor
from dtw_standalone.analysis.phase_discovery import PhaseDiscoverer
from dtw_standalone.analysis.fsm_counter import FSMCounter
from dtw_standalone.analysis.dtw_aligner import DTWAligner
from dtw_standalone.utils.video_muxer import VideoMuxer
...
class AIFitnessPipeline:
    ...
    def process_student_video(self, video_path, output_path):
        ...
        cap.release()
        muxer.close()
        self.last_frames_written = frame_idx
        print(f"Output saved to {output_path}")
        
def run_backtest(teacher_video: str, student_video: str, output_video: str) -> tuple[str, dict]:
    import time
    start = time.perf_counter()
    pipeline = AIFitnessPipeline()
    pipeline.process_teacher_video(teacher_video)
    pipeline.process_student_video(student_video, output_video)
    
    # Transcode to H.264 for browser playback
    temp_output_path = output_video + ".temp.mp4"
    if os.path.exists(output_video):
        os.rename(output_video, temp_output_path)
        import subprocess
        subprocess.run([
            "ffmpeg", "-nostdin", "-y", "-i", temp_output_path, 
            "-vcodec", "libx264", "-pix_fmt", "yuv420p", "-preset", "fast", "-crf", "23", 
            "-movflags", "+faststart", "-an", output_video
        ], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if os.path.exists(temp_output_path):
            os.remove(temp_output_path)

    avg_score = float(np.mean(pipeline.last_scores)) if pipeline.last_scores else 0.0
    stats = {
        "score": avg_score,
        "rep_count": pipeline.last_rep_count,
        "frames_written": pipeline.last_frames_written,
        "avg_normalized_dtw": 1.0 - avg_score,
        "avg_phase_error": 0.0,
