...
class DTWBacktestRequest(BaseModel):
    template_video_uri: str
    student_video_uri: str
    mode: TrackMode = "reps"
    observed_rep_count: int | None = Field(default=None, ge=0)
    template_name: str | None = None
    template_trim_start_sec: float | None = Field(default=None, ge=0.0)
    template_trim_end_sec: float | None = Field(default=None, ge=0.0)
    include_llm_feedback: bool = False
    include_visual_sync: bool = True
...
class DTWBacktestResponse(BaseModel):
    backtest_id: str
    template_video_uri: str
    student_video_uri: str
    result: dict[str, Any]
