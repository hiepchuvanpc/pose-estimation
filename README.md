...
3. Chay API (ban chinh thuc)

   Set-Location -LiteralPath 'F:\ACV\new folder\APP2\app'
   $env:PYTHONPATH='F:\ACV\new folder\APP2\app\src'
   python -m uvicorn motion_api.main:app --reload --reload-dir 'F:\ACV\new folder\APP2\app\src' --reload-dir 'F:\ACV\new folder\APP2\app\web'

   Hoac chay script:
   powershell -ExecutionPolicy Bypass -File scripts/run_web_official.ps1
...
## Gemini LLM feedback (optional)

- Tao file `.env` tai thu muc `app/` va them:

  `GEMINI_API_KEY=<YOUR_GEMINI_KEY>`

- API se tu doc `.env` khi khoi dong (khong can set thu cong trong PowerShell).
- Neu vua co env he thong vua co `.env`, env he thong duoc uu tien.

- Endpoint moi: `POST /v1/analysis/llm-feedback`
- Muc tieu: nhan `segment` da tinh toan tu engine rule/DTW, sau do Gemini viet nhan xet de doc de hieu hon.

- Ban cung co the goi 1 lan trong backtest:
  - `POST /v1/analysis/dtw-backtest`
  - them `include_visual_sync: true` de bat buoc render video so khop truc quan.
  - them `include_llm_feedback: true` trong payload de nhan them truong `llm_feedback`.

Payload mau:

```json
{
  "segment": { "exercise_name": "Squat", "mode": "reps", "scoring": { "overall": 72.5 }, "reference_plane": { "camera_view_mismatch": false, "mirror_aligned": true }, "joint_analyses": [], "rep_feedback": [] },
  "fitness_level": "beginner",
  "user_history_summary": "Hay lech goi trai o cuoi set",
...
```bash
curl -X POST "http://127.0.0.1:8000/v1/analysis/llm-feedback" \
