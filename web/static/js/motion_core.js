...
class SignalNormalizer {
    ...
    normalize(raw) {
        ...
        if (span < 0.25) { // Minimum ROM required to normalize! Otherwise return raw
            return s;
        }
...
class RepCounter {
    constructor() {
        ...
        // Add normalizer to dynamically adjust to the user's amplitude range (like find_peaks)
        this.normalizer = new SignalNormalizer();
        this.normalizer.warmup = 3;
    }
    update(rawSignal, timestampMs) {
        const s = this.normalizer.normalize(rawSignal);
