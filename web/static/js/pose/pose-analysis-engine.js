...
export class PoseAnalysisEngine {
  ...
  readinessFromLandmarks(landmarks, profile = null, score = null, mode = null, templateId = null) {
    ...
    if (!strictViewReady) {
      ...
      const fallbackSimilarityMin = Math.max(0.32, cfg.similarityMin);
      const fallbackMeanMin = Math.max(0.3, cfg.meanMin);
      // Soft fallback: only allow start when body is visible enough and similarity is acceptable.
      if (!(similarity >= fallbackSimilarityMin && history.mean >= fallbackMeanMin && trunk >= 0.55 && lower >= 0.42)) {
        return false;
      }
    }
